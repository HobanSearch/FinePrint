...
import type { PatternLibraryEntry, PatternMatch, TextMatch } from '@fineprintai/shared-types';

const logger = createServiceLogger('pattern-library');
...
export class PatternLibrary {
  ...
  private addPattern(pattern: PatternLibraryEntry): void {
    this.patterns.set(pattern.id, pattern);
  }

  async analyzeText(text: string): Promise<PatternAnalysisResult> {
  ...
  private extractContext(text: string, start: number, length: number, contextLength: number = 150): string {
  ...
  private calculateConfidence(matches: TextMatch[], pattern: PatternLibraryEntry): number {
    ...
    const exactMatches = matches.filter(match => 
      pattern.patternKeywords.some(keyword => 
        match.text.toLowerCase().includes(keyword.replace(/\.\*/g, ''))
      )
    );
  ...
  private categorizeMatches(matches: PatternMatch[]): { [category: string]: PatternMatch[] } {
    const categorized: { [category: string]: PatternMatch[] } = {};
  ...
  private calculateRiskScore(matches: PatternMatch[]): number {
    if (matches.length === 0) return 0;
  ...
  private getHighestSeverity(matches: PatternMatch[]): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private calculateAffectedTextPercentage(matches: PatternMatch[], totalTextLength: number): number {
    if (totalTextLength === 0) return 0;

    const affectedChars = new Set<number>();
    ...
    return Math.round((affectedChars.size / totalTextLength) * 10000) / 100; // Two decimal places
  }
  ...
  addCustomPattern(pattern: Omit<PatternLibraryEntry, 'id' | 'isCustom' | 'version'>): string {
    ...
    const customPattern: PatternLibraryEntry = {
      ...pattern,
      id,
      isCustom: true,
      version: 1
