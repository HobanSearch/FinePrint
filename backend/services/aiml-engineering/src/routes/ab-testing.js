...
const logger = (0, logger_1.createServiceLogger)('ab-testing-api');
const abTestingRoutes = async (fastify) => {
    const prisma = new client_1.PrismaClient();
    const abTestingService = new ab_testing_service_1.ABTestingService(prisma);
    fastify.post('/create', {
        schema: {
            description: 'Create a new A/B test',
            tags: ['A/B Testing'],
            body: ab_testing_service_1.ABTestConfigSchema,
            response: {
                200: {
                    type: 'object',
                    properties: {
    ...
    fastify.post('/:testId/stop', {
        ...
        try {
            const { testId } = request.params;
            const { reason = 'Manual stop' } = request.body;
...
exports.default = abTestingRoutes;
