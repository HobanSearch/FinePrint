/**
 * Fine Print AI - Model Evaluation API Routes
 */

import { FastifyPluginAsync } from 'fastify';
import { PrismaClient } from '@prisma/client';
import { ModelEvaluationService, EvaluationConfigSchema } from '../services/model-evaluation-service';
import { createServiceLogger } from '@fineprintai/shared-logger';

const logger = createServiceLogger('model-evaluation-api');

const modelEvaluationRoutes: FastifyPluginAsync = async (fastify) => {
  const prisma = new PrismaClient();
  const evaluationService = new ModelEvaluationService(prisma);

  /**
   * Start new model evaluation
   */
  fastify.post('/start', {
    schema: {
      description: 'Start a new model evaluation',
      tags: ['Model Evaluation'],
      body: EvaluationConfigSchema,
      response: {
        200: {
          type: 'object',
          properties: {
  ...
  fastify.get('/:evaluationId', {
    ...
    try {
      const { evaluationId } = request.params as any;
  ...
  function calculateOverallMetrics(results: any[]) {
    ...
    metricKeys.forEach(metric => {
      ...
      if (values.length > 0) {
        avgMetrics[`avg_${metric}`] = values.reduce((sum: number, v: number) => sum + v, 0) / values.length;
  ...
  fastify.get('/:evaluationId/download', {
    ...
    try {
      ...
      reply.header('Content-Type', contentTypes[format as keyof typeof contentTypes]);
  ...
  /**
   * Get evaluation metrics comparison
   */
  fastify.get('/:evaluationId/comparison', {
    ...
    try {
      ...
      // Enhanced comparison with model-specific breakdowns
  ...
  function calculateModelScore(result: any): number {
    let score = 0;
  ...
  function identifyModelStrengths(result: any): string[] {
    const strengths: string[] = [];
    if (result.metrics.accuracy && result.metrics.accuracy > 0.9) strengths.push('High accuracy');
    if (result.metrics.f1_score && result.metrics.f1_score > 0.85) strengths.push('Excellent F1 score');
    if (result.performance_stats.avg_response_time < 500) strengths.push('Fast response time');
    const errorRate = result.performance_stats.failed_predictions / result.performance_stats.total_predictions;
    if (errorRate < 0.02) strengths.push('Low error rate');
    return strengths;
  }

  function identifyModelWeaknesses(result: any): string[] {
    const weaknesses: string[] = [];
    if (result.metrics.accuracy && result.metrics.accuracy < 0.7) weaknesses.push('Low accuracy');
    if (result.performance_stats.avg_response_time > 1500) weaknesses.push('Slow response time');
    const errorRate = result.performance_stats.failed_predictions / result.performance_stats.total_predictions;
    if (errorRate > 0.1) weaknesses.push('High error rate');
    if (result.metrics.precision && result.metrics.precision < 0.6) weaknesses.push('Low precision');
    return weaknesses;
  }

  function createMetricBreakdown(results: any[]) {
    const breakdown: any = {};
  ...
  fastify.get('/:evaluationId/recommendations', {
    ...
    try {
      const { evaluationId } = request.params as any;
      const { priority, type } = request.query as any;
...
export default modelEvaluationRoutes;