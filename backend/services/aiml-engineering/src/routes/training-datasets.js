...
const training_dataset_generator_1 = require("../services/training-dataset-generator");
const logger_1 = require("@fineprintai/shared-logger");
const logger = (0, logger_1.createServiceLogger)('training-datasets-api');
const trainingDatasetsRoutes = async (fastify) => {
    const prisma = new client_1.PrismaClient();
    const datasetGenerator = new training_dataset_generator_1.TrainingDatasetGenerator(prisma);
    fastify.post('/generate', {
        schema: {
            description: 'Generate a new training dataset from aggregated documents',
            tags: ['Training Datasets'],
            body: training_dataset_generator_1.DatasetConfigSchema,
            response: {
                200: {
                    type: 'object',
                    properties: {
    ...
    fastify.get('/', {
        ...
        try {
            const { task_type, jurisdiction, limit = 20, offset = 0 } = request.query;
    ...
    fastify.get('/:datasetId/download/:split', {
        ...
        try {
            const { datasetId, split } = request.params;
            const dataset = await datasetGenerator.getDataset(datasetId);
            if (!dataset) {
                return reply.status(404).send({
                    success: false,
            ...
            const filePath = dataset.file_paths[split];
            if (!filePath) {
                return reply.status(400).send({
                    success: false,
    ...
    fastify.post('/validate-config', {
        ...
        try {
            const config = request.body;
            const validation = training_dataset_generator_1.DatasetConfigSchema.safeParse(config);
...
exports.default = trainingDatasetsRoutes;
//# sourceMappingURL=training-datasets.js.map