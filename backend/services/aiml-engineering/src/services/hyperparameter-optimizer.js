...
exports.HyperparameterOptimizer = exports.OptimizationConfigSchema = exports.SearchSpaceSchema = void 0;
...
const logger = (0, logger_1.createServiceLogger)('hyperparameter-optimizer');
exports.SearchSpaceSchema = zod_1.z.object({
    learning_rate: zod_1.z.object({
        type: zod_1.z.enum(['float', 'categorical']),
        min: zod_1.z.number().optional(),
        max: zod_1.z.number().optional(),
        values: zod_1.z.array(zod_1.z.number()).optional(),
        log: zod_1.z.boolean().default(true),
    }).optional(),
    batch_size: zod_1.z.object({
        type: zod_1.z.enum(['int', 'categorical']),
        min: zod_1.z.number().optional(),
        max: zod_1.z.number().optional(),
        values: zod_1.z.array(zod_1.z.number()).optional(),
        step: zod_1.z.number().optional(),
    }).optional(),
    num_epochs: zod_1.z.object({
        type: zod_1.z.enum(['int']),
        min: zod_1.z.number().min(1),
        max: zod_1.z.number().max(1000),
        step: zod_1.z.number().default(1),
    }).optional(),
    weight_decay: zod_1.z.object({
        type: zod_1.z.enum(['float', 'categorical']),
        min: zod_1.z.number().optional(),
        max: zod_1.z.number().optional(),
        values: zod_1.z.array(zod_1.z.number()).optional(),
    ...
    dropout_rate: zod_1.z.object({
        type: zod_1.z.enum(['float']),
        min: zod_1.z.number().min(0),
        max: zod_1.z.number().max(1),
    }).optional(),
    hidden_size: zod_1.z.object({
        type: zod_1.z.enum(['int', 'categorical']),
        min: zod_1.z.number().optional(),
        max: zod_1.z.number().optional(),
        values: zod_1.z.array(zod_1.z.number()).optional(),
    }).optional(),
    num_layers: zod_1.z.object({
        type: zod_1.z.enum(['int']),
        min: zod_1.z.number().min(1),
        max: zod_1.z.number().max(50),
    }).optional(),
    optimizer: zod_1.z.object({
        type: zod_1.z.enum(['categorical']),
        values: zod_1.z.array(zod_1.z.enum(['adamw', 'adam', 'sgd', 'adafactor'])),
    }).optional(),
    scheduler: zod_1.z.object({
        type: zod_1.z.enum(['categorical']),
        values: zod_1.z.array(zod_1.z.enum(['linear', 'cosine', 'polynomial', 'constant'])),
    }).optional(),
    model_specific: zod_1.z.record(zod_1.z.any()).optional(),
});
exports.OptimizationConfigSchema = zod_1.z.object({
    study_name: zod_1.z.string().min(1),
    ...
    search_space: exports.SearchSpaceSchema,
    optimization_settings: zod_1.z.object({
        sampler: zod_1.z.enum(['tpe', 'random', 'cmaes', 'grid']).default('tpe'),
        pruner: zod_1.z.enum(['median', 'hyperband', 'successive_halving', 'none']).default('median'),
        direction: zod_1.z.enum(['minimize', 'maximize']).default('maximize'),
        n_trials: zod_1.z.number().min(1).max(10000).default(100),
        timeout: zod_1.z.number().min(60).optional(),
        n_jobs: zod_1.z.number().min(1).max(16).default(1),
        load_if_exists: zod_1.z.boolean().default(true),
    }),
    objective_metric: zod_1.z.enum(['accuracy', 'f1_score', 'precision', 'recall', 'loss', 'auc', 'custom']).default('f1_score'),
    multi_objective: zod_1.z.object({
    ...
    early_stopping: zod_1.z.object({
        ...
        patience: zod_1.z.number().min(1).default(10),
        min_improvement: zod_1.z.number().min(0).default(0.001),
    }),
    resource_constraints: zod_1.z.object({
        max_gpu_memory_gb: zod_1.z.number().min(1).default(8),
        max_training_time_hours: zod_1.z.number().min(0.1).default(24),
        max_concurrent_trials: zod_1.z.number().min(1).default(2),
    }),
    validation_strategy: zod_1.z.object({
        method: zod_1.z.enum(['holdout', 'k_fold', 'time_series']).default('holdout'),
        k_folds: zod_1.z.number().min(2).max(20).default(5),
        validation_split: zod_1.z.number().min(0.1).max(0.5).default(0.2),
    }),
});
class HyperparameterOptimizer extends events_1.EventEmitter {
    cache;
    queue;
    modelRegistry;
    performanceMonitor;
    ...
    async loadActiveStudies() {
        try {
            ...
            if (cachedStudies) {
                const studies = JSON.parse(cachedStudies);
    ...
    async setupOptimizationScripts() {
...
`;
    ...
    async startOptimization(config) {
        try {
            const validatedConfig = exports.OptimizationConfigSchema.parse(config);
            const studyId = (0, uuid_1.v4)();
            const study = {
                id: studyId,
    ...
    async processOptimizationStudy(studyData) {
    ...
    async monitorOptimizationProcess(study, process, studyDir) {
    ...
    parseOptimizationOutput(study, output) {
        try {
    ...
    startOptimizationResourceMonitoring(study) {
        const monitor = setInterval(async () => {
            try {
                ...
                study.resource_usage.total_gpu_hours += usage.gpu_utilization / 3600;
                study.resource_usage.total_cpu_hours += usage.cpu_utilization / 3600;
                study.resource_usage.peak_memory_gb = Math.max(study.resource_usage.peak_memory_gb, usage.memory_used_mb / 1024);
                const gpu_cost_per_hour = 2.0;
                const cpu_cost_per_hour = 0.1;
    ...
    waitForOptimizationCompletion(process, study, studyDir) {
    ...
    async processOptimizationResults(study, studyDir) {
        try {
            ...
            if (await fs.pathExists(resultsPath)) {
                ...
                study.trials = results.trials.map((trialData) => ({
                    id: (0, uuid_1.v4)(),
    ...
    async stopOptimization(studyId) {
        const study = this.activeStudies.get(studyId);
        if (!study) {
            throw new Error(`Optimization study ${studyId} not found`);
        }
    ...
    async updateActiveStudiesCache() {
        try {
    ...
    async startMultiObjectiveOptimization(config) {
    ...
    async startBayesianOptimization(config, acquisitionFunction = 'ei') {
    ...
    async startHyperbandOptimization(config) {
    ...
    async generateOptimizationReport(studyId) {
        const study = this.activeStudies.get(studyId);
        if (!study) {
    ...
    calculateParameterImportance(trials) {
        const importance = {};
        if (trials.length < 2)
            return importance;
    ...
    getCumulativeBest(trials, currentTrialNumber, direction) {
...
//# sourceMappingURL=hyperparameter-optimizer.js.map