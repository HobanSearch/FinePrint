...
export async function designSystemRoutes(fastify) {
    fastify.post('/tokens', {
        schema: {
            tags: ['Design Tokens'],
    ...
    fastify.get('/tokens/search', {
        ...
        const { q, category, limit = 50, offset = 0 } = request.query;
        let tokens = [];
        if (q) {
    ...
    fastify.post('/tokens/bulk', {
        ...
        const results = await Promise.allSettled(tokens.map(token => fastify.designSystemEngine.createToken(token)));
    ...
    fastify.get('/analytics/usage', {
        ...
        const { period = 'week', tokenId, componentId } = request.query;
        const analytics = {
            period,
...
async function getTokenUsage(tokenId, period) {
    return {
        tokenId,
        period,
        usage: [
...
async function getComponentUsage(componentId, period) {
    return {
        componentId,
        period,
        usage: [
