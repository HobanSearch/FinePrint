...
export async function componentGenerationRoutes(fastify) {
    ...
    fastify.post('/variant', {
        ...
        try {
            const variant = await fastify.componentGenerator.generateVariant(baseComponentId, variantName, modifications);
            reply.code(201).send({
                success: true,
                data: variant,
            });
    ...
    fastify.get('/templates', {
        ...
        const { framework, type } = request.query;
        const templates = [
            {
                id: 'button-template',
    ...
    fastify.get('/components', {
        ...
        const { framework, type, limit = 20, offset = 0 } = request.query;
...
async function getTemplate(templateId, framework) {
    return {
...
function calculateEstimatedSize(spec) {
    let lines = 20;
    lines += Object.keys(spec.props || {}).length * 2;
    lines += spec.variants?.length * 5 || 0;
    lines += spec.accessibility ? 10 : 0;
    lines += spec.responsive ? 8 : 0;
    return {
        lines,
        bytes: lines * 50,
    };
}
function estimateDependencies(spec) {
    const deps = [];
    if (spec.framework === 'react') {
        deps.push('react');
        if (spec.accessibility)
            deps.push('@radix-ui/react-accessible-icon');
        if (spec.styling?.cssModules)
            deps.push('clsx');
    }
...
function extractAccessibilityFeatures(accessibility) {
    if (!accessibility)
        return [];
    const features = [];
    ...
    if (accessibility.role)
        features.push(`ARIA Role: ${accessibility.role}`);
    if (accessibility.ariaLabel)
        features.push('ARIA Label');
    return features;
}
function generateCodePreview(spec) {
    if (spec.framework === 'react') {
...
async function getGeneratedComponent(id) {
    return null;
}
async function listGeneratedComponents(filters) {
    return {
        components: [],
        total: 0,
        limit: filters.limit,
...
async function createComponentZip(component) {
    return Buffer.from('mock-zip-content');
}
//# sourceMappingURL=component-generation.js.map