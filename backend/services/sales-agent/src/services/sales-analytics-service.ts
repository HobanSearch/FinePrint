...
import { BusinessMetrics, Lead, Opportunity } from '@fineprintai/shared-types';
...
export class SalesAnalyticsService {
  ...
  private async getPipelineMetrics(): Promise<any> {
    ...
    const totalWeightedValue = weightedPipeline.reduce(
  ...
  private async getTrendMetrics(): Promise<any> {
  ...
  private async calculateAverageSalesCycle(): Promise<number> {
    ...
    const totalDays = closedOpportunities.reduce((sum, opp) => {
      const days = Math.floor(
        (opp.actualCloseDate!.getTime() - opp.createdAt.getTime()) / (24 * 60 * 60 * 1000)
      );
      return sum + days;
    }, 0);
  ...
  private async getConversionRates(): Promise<any> {
  ...
  private calculateGrowthRates(monthlyData: any[]): any {
    const growthRates = {
      leads: [],
      revenue: [],
      opportunities: [],
  ...
  async getLeadSourceAnalysis(): Promise<any> {
    ...
    const sourceConversions = await Promise.all(
  ...
  async getActivityAnalysis(): Promise<any> {
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    ...
    const activityEffectiveness = await Promise.all(
      activityData.map(async (activity) => {
        // Count leads that moved to next stage after this activity type
        ...
        return {
          ...
          effective_activities: (effectiveActivities as any)[0]?.effective_count || 0,
          effectiveness_rate: activity._count > 0 
            ? ((effectiveActivities as any)[0]?.effective_count || 0) / activity._count * 100 
            : 0,
        };
      })
    );
  ...
  // Public API methods for external consumption
  async exportAnalyticsData(format: 'json' | 'csv' = 'json'): Promise<any> {
