...
const resolvers = {
  Query: {
    knowledgeGraphStats: async (root: any, args: any, context: any) => {
    ...
    searchConcepts: async (root: any, args: { query: string; limit?: number }, context: any) => {
      ...
      return concepts.slice(0, args.limit || 10).map((concept: any) => ({
        id: concept.id,
        name: concept.name,
    ...
    learnerProfile: async (root: any, args: { learnerId: string }, context: any) => {
    ...
    graphAnalytics: async (root: any, args: any, context: any) => {
  ...
  Mutation: {
    createLearnerProfile: async (root: any, args: { input: any }, context: any) => {
    ...
    extractKnowledge: async (root: any, args: { input: any }, context: any) => {
    ...
    performInference: async (root: any, args: { input: any }, context: any) => {
...
export async function GraphQLSchema(server: FastifyInstance): Promise<void> {
  await server.register(mercurius, {
    schema,
    resolvers,
    context: (request) => ({
      knowledgeGraph: (server as any).knowledgeGraph,
      curriculumLearning: (server as any).curriculumLearning,
      graphAnalytics: (server as any).graphAnalytics,
    }),
