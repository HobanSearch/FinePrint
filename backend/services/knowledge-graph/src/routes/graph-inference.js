...
exports.graphInferenceRoutes = graphInferenceRoutes;
async function graphInferenceRoutes(server) {
    ...
    server.post('/batch-enhanced', {
        ...
        try {
            const { GraphEnhancedInferenceService } = await import('../services/graph-enhanced-inference-service');
            const inferenceService = new GraphEnhancedInferenceService(knowledgeGraph);
            const results = await inferenceService.batchEnhancedInference(request.body.requests, request.body.share_context !== false);
