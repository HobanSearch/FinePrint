...
import { KnowledgeQuery, GraphReasoningRequest } from '../services/knowledge-graph-service';

export async function knowledgeGraphRoutes(server: FastifyInstance): Promise<void> {
  ...
  }, async (request: FastifyRequest<{ Body: KnowledgeQuery }>, reply: FastifyReply) => {
    try {
  ...
  }, async (request: FastifyRequest<{ Body: GraphReasoningRequest }>, reply: FastifyReply) => {
    try {
